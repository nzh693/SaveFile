...
​    锁实现：关键字synchronized、lock的实现类   使用场景：写操作多。

##### 3、乐观锁

​    认为在操作数据的时候不会被其他线程修改数据，所有不会加锁，只是在更新数据时会去判断有没有其他线程更新了这个数据；   

​    锁实现：CAS（compare and swap）算法    使用场景：读操作较多，不加锁性能有所大幅度提高。
...
​             根据一定算法获取上次自旋成功的次数进行调整最大自旋次数。不会存在死锁的情况。

​    **为什么会进行自旋**

阻塞与唤醒线程需要保持和恢复现场，系统会切换cpu的状态，从用户态度，内核态。需要消耗资源和时间。
...
**String**的值是不可变的，这就导致每次对String的操作都会生成**新的String对象**，不仅效率低下，而且大量浪费有限的内存空间。字面量	` String test="abc"`赋值，此时的字符串声明在字符串常量池中【StringTable】，new 的形式则在堆创建一个对象。
